...
# A live-only miss falls back to qstat -x -f in the same SSH invocation.
...
assert_eq "$(cat "$historical_dir/state")" terminal "PBS historical terminal state"
assert_grep "$historical_dir/detail" 'qstat-x=.*Exit_status = 0' "PBS historical Exit_status detail"
assert_grep "$MOCK_DIR/calls.log" 'qstat -x -f.*124.server' "PBS qstat -x -f invocation"
